...
int checkJugada(coord pcActual,coord pcSiguiente,int turno,int colorJugador,char* nombre){
        ...
        if (turno==PERSONA){
            ...
            do
            {
                ...
                if(((absDiffI == 2 && absDiffJ == 2) || (absDiffI == 0 && absDiffJ == 2) || (absDiffI == 2 && absDiffJ == 0)) && cont==0){
                    ...
                    tablero[coord[2]][coord[3]] = (colorJugador = BLANCO?'B':'N');
    	...
    	if(turno == PERSONA){
			...
			do{
				  ...
				  if(tablero[jugadorActual.x][jugadorActual.y] == (colorJugador = BLANCO?'B':'N'))
					  bandera = checkJugada(jugadorActual,jugadorSiguiente,turno,colorJugador, nombre);
			}while(bandera == 0);
			...
			do{
			  ...
			  if(tablero[pcActual.x][pcActual.y] == (colorJugador = NEGRO?'B':'N'))
				  bandera =  checkJugada(pcActual,pcSiguiente,turno,colorJugador,nombre);
			}while(bandera == 0);
