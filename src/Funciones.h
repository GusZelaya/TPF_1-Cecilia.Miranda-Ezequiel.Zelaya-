...
void CrearEstadisticas()
{
	...
	fwrite(&GuardadoStats,sizeof(StructEstadisticas),1,Archivo);
	fclose(Archivo);
}
...
void mostrar(GtkWidget *event_box, GdkEventButton *event, gpointer data)
{
	    ...
	    if ((a=fopen(RutaEstadisticas,"r"))==NULL)
	    {
	    	...
	    	StructEstadisticas GuardadoStats;
	    	fread(&GuardadoStats,sizeof(StructEstadisticas),1,a);
	    	gchar * str=Juego.username;
	    	strcat(str,&GuardadoStats);
	    	    if (str != NULL)
	    	    {
...
{
	gtk_widget_destroy(VentanaCreacionPartida);
